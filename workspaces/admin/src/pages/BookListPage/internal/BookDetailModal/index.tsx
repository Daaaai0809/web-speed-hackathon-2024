...
export const BookDetailModal: React.FC<Props> = ({ bookId, isOpen, onClose }) => {
  ...
  const [isEdit, toggleIEdit] = useState(false);
      ...
      <ModalContent containerProps={{ p: 8 }} height="100%" m={0} overflowY="auto">
        ...
        <Stack height="100%" p={4}>
          {book != null && (
            <>
              {isEdit ? (
                <BookEditContent book={book} onEditComplete={() => toggleIEdit(!isEdit)} />
              ) : (
                <BookDetailContent book={book} onCloseDialog={onClose} onEdit={() => toggleIEdit(!isEdit)} />
              )}
            </>
          )}
