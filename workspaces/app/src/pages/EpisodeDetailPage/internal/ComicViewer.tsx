...
export const ComicViewer: React.FC<Props> = ({ episodeId }) => {
  ...
  // ビュアーの高さ
  let viewerHeight = candidatePageHeight;
  if (viewerHeight < MIN_VIEWER_HEIGHT) {
    viewerHeight = MIN_VIEWER_HEIGHT;
  } else if (viewerHeight > MAX_VIEWER_HEIGHT) {
  ...
  return (
    <_Container ref={ref}>
      <_Wrapper $maxHeight={viewerHeight}>
        <ComicViewerCore episodeId={episodeId} height={650} />
      </_Wrapper>
    </_Container>
  );
};
